...
if (composeData.services) {
  ...
  if (composeData.services['claude-code']) {
    ...
    if (claudeCode.environment && Array.isArray(claudeCode.environment)) {
      ...
      claudeCode.environment = claudeCode.environment.filter((env: string) => 
        !env.includes('MCP_GATEWAY_URL')
      );
    ...
    if (claudeCode.volumes && Array.isArray(claudeCode.volumes)) {
      ...
      claudeCode.volumes = claudeCode.volumes.filter((vol: string) => 
    ...
    if (claudeCode.extra_hosts && Array.isArray(claudeCode.extra_hosts)) {
      ...
      claudeCode.extra_hosts = claudeCode.extra_hosts.filter((host: string) => 
        !host.includes('shared-mcp-gateway-server')
      );
...
if (fs.existsSync(envPath)) {
  ...
  const mcpVars = ['MCP_PROXY_PORT', 'MCP_API_PORT', 'MCP_WEB_PORT', 'MCP_CONFIG_PATH'];
