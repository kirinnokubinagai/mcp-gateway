/// <reference path="./types/mcp-sdk.d.ts" />
...
export class ConnectionPool extends EventEmitter {
  ...
  async acquire(config: ConnectionConfig): Promise<PooledConnection> {
    ...
    const availableConnection = existingConnections.find(
      (conn) => !this.activeConnections.has(conn) && conn.isHealthy
    );
    ...
    if (
      totalConnections >= this.options.maxTotalConnections ||
  ...
  private async createConnection(config: ConnectionConfig): Promise<PooledConnection> {
    ...
    try {
      const transport = new WebSocketTransport({
        ...
        reconnectAttempts: 3,  // 再接続を有効化
        pingInterval: 15000,   // 30秒から15秒に短縮して接続維持
    ...
    this.idleCheckInterval = setInterval(async () => {
      ...
      for (const [key, connections] of this.connections) {
        const idleConnections = connections.filter(
          (conn) =>
            !this.activeConnections.has(conn) && now - conn.lastUsedAt > this.options.idleTimeout
        );
