...
import { Config, ServerConfig } from './types.ts';
import {
  configSchema,
...
import { migrateConfig, compareVersions } from './schemas/migrations.ts';
...
export class ConfigValidator {
  ...
  async validateConfig(config: any, isProfileConfig: boolean = false): Promise<ValidationResult> {
    ...
    console.log('[ConfigValidator] validateConfig called with:', {
    ...
    try {
      ...
      if (typeof config !== 'object' || config === null) {
        console.error('[ConfigValidator] 設定が無効:', {
          type: typeof config,
          isNull: config === null,
          config: config
        });
  ...
  private async migrateConfigIfNeeded(config: any): Promise<any> {
    ...
    if (compareVersions(currentVersion, CONFIG_SCHEMA_VERSION) < 0) {
      console.log(
        `設定をバージョン ${currentVersion} から ${CONFIG_SCHEMA_VERSION} へマイグレーションします`
      );
          ...
          for (const sensitiveKey of validationRules.env.sensitiveKeys) {
            if (envKey.toUpperCase().includes(sensitiveKey)) {
              warnings.push({
                ...
                suggestion:
                  '機密情報は環境変数ファイルや秘密管理システムで管理することを推奨します',
              });
            }
          }
  ...
  private normalizeConfig(config: any, isProfileConfig: boolean): Config {
    ...
    for (const [name, server] of Object.entries(servers)) {
      ...
      // commandが存在し、文字列であることを確認
      if (!serverConfig.command || typeof serverConfig.command !== 'string') {
        console.warn(`サーバー '${name}' のcommandが無効です。スキップします。`);
        continue;
      }
      ...
      // 環境変数の展開をマーク
      if (normalizedServers[name] && normalizedServers[name].env) {
