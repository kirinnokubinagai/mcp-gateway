...
import { StateManager, ServerState } from './state-manager.ts';
...
import { createLogger, setupGlobalErrorHandlers } from './logger.ts';
import { ConnectionPool, ConnectionConfig } from './connection-pool.ts';
import { ServerConfig, ProfileConfig, Config, MCPClientInfo, Tool } from './types.ts';
import {
  measurePerformance,
  ParallelExecutor,
  configCache,
...
const connectionPool = new ConnectionPool({
  ...
  connectionTimeout: 120000,  // 30秒から120秒に延長
  idleTimeout: 600000,        // 5分から10分に延長
  healthCheckIntervalMs: 120000,  // 60秒から120秒に延長
  maxHealthCheckFailures: 5,      // 3回から5回に増加
  queueTimeout: 180000,           // 60秒から180秒に延長
});
...
async function loadConfig(): Promise<Config> {
  ...
  try {
    ...
    logger.debug('読み込んだ設定', { 
      configType: typeof rawConfig,
      isNull: rawConfig === null,
      isArray: Array.isArray(rawConfig),
      keys: rawConfig ? Object.keys(rawConfig).slice(0, 5) : 'N/A'
    });
    ...
    if (process.env.MCP_PROFILE) {
      ...
      logger.info(`環境変数からプロファイルを設定: ${process.env.MCP_PROFILE}`, {
...
function isServerEnabledForProfile(serverName: string, config: Config): boolean {
  ...
  if (serverName in profile) {
    return profile[serverName] ?? false;
  }
...
function extractDockerImageName(args: string[]): string | null {
  ...
  for (let i = runIndex + 1; i < args.length; i++) {
    ...
    if (arg && arg.startsWith('-')) {
      ...
      if (arg && optionsWithValue.has(arg)) {
        i++;
...
async function connectToMCPServer(
  ...
  try {
    ...
    if (transport instanceof WebSocketTransport) {
      ...
      transport.onclose = async () => {
        ...
        if (clientInfo && clientInfo.status === 'connected') {
          ...
          // Dockerコンテナの場合、WebSocket切断は正常な動作の可能性がある
          if (config.command === 'docker') {
            logger.info(`Docker接続が切断されました - コンテナの状態を確認中`, { serverName: name });
            ...
            try {
              // コンテナ名を取得（プロキシサーバーと同じ命名規則）
              const imageName = config.args?.find((arg: string) => arg.includes(':') && !arg.startsWith('-')) || '';
              const baseName = imageName.replace(/[/:]/g, '-').replace(/^-+|-+$/g, '');
              const containerName = `mcp-proxy-${baseName}`;
              
              // コンテナの状態を確認
              const result = execSync(`docker inspect -f '{{.State.Running}}' ${containerName} 2>/dev/null || echo false`, { encoding: 'utf8' }).trim();
            ...
            if (containerRunning) {
              // コンテナは実行中なので、単なる一時的な切断として扱う
              logger.info(`Dockerコンテナは実行中です - 接続を維持`, { serverName: name });
              return; // エラー状態にせず、接続を維持
            }
          }
          
          // Docker以外、またはコンテナが実際に停止している場合
          mcpClients.set(name, {
            ...clientInfo,
            status: 'error',
            error: 'サーバーが終了しました',
          ...
          // エラー情報を作成
          const errorInfo = createErrorInfo('サーバーが終了しました', name);
          recordFailure(name, errorInfo);

          // 復旧戦略を実行（サーキットブレーカーとバックオフ付き）
          if (errorInfo.retryable) {
            const errorStatus = getErrorStatus(name, 'サーバーが終了しました');
    ...
    try {
      ...
      // 初回接続時のみ詳細ログを出力
      if (!existingClient || existingClient.status !== 'error') {
        logger.debug(`ツール取得成功: ${tools.length}個`, {
          serverName: name,
    ...
    if (errorMessage.includes('タイムアウト') || errorMessage.includes('timeout')) {
      ...
      userFriendlyMessage =
        'プロキシサーバーに接続できません。プロキシサーバーが起動しているか確認してください。';
      logger.error(userFriendlyMessage, error, {
        serverName: name,
        errorType: 'connection_refused',
      });
...
mcpServer.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === 'host_execute_command') {
    ...
    if (!allowedCommands.includes(commandName)) {
      throw new Error(
        `許可されていないコマンド: ${commandName}。許可されているコマンド: ${allowedCommands.join(', ')}`
      );
    }
    ...
    try {
      ...
      const proxyUrl = process.env.DOCKER_ENV ? 'ws://host.docker.internal:9999' : 'ws://localhost:9999';
  ...
  if (client.toolMapping?.has(name)) {
...
async function disconnectFromMCPServer(name: string) {
  ...
  if (clientInfo) {
    try {
      ...
      } else if (clientInfo.client && typeof clientInfo.client.close === 'function') {
        // レガシー接続の場合は直接クローズ
...
async function syncWithConfig() {
  ...
  for (const [currentName, clientInfo] of mcpClients.entries()) {
    if (!configServers.has(currentName)) {
      ...
      for (const [configName, serverConfig] of Object.entries(config.mcpServers)) {
        ...
        // Dockerコンテナの場合は特別な処理
        if (
          clientInfo.config.command === 'docker' &&
          serverConfig.command === 'docker' &&
          clientInfo.config.args?.includes('run') &&
          serverConfig.args?.includes('run')
        ) {
  ...
  for (const [name, serverConfig] of Object.entries(config.mcpServers)) {
    ...
    if (isEnabled) {
      ...
      // 設定は同じでエラー状態の場合はそのままにする（自動再接続はしない）
      else if (currentClient.status === 'error') {
        statusChanges.unchanged.push(name);
        logger.debug(`エラー状態のサーバーの再接続をスキップ`, { serverName: name });
      }
      // 設定が同じで接続も正常な場合
      else {
        statusChanges.unchanged.push(name);
  ...
  if (totalTasks === 0) {
    ...
    if (connectTasks.length > 0) {
      ...
      await ParallelExecutor.mapConcurrent(connectTasks, async (task) => await task, concurrency);
    }
  }
  ...
  configWatcher = watch(CONFIG_FILE, async (eventType) => {
    if (eventType === 'change') {
      ...
      reloadTimeout = setTimeout(async () => {
        ...
        // 変更された設定を検証
        const newConfigData = await fs.readFile(CONFIG_FILE, 'utf-8');
        const newConfig = JSON.parse(newConfigData);
        const validationResult = await configValidator.validateConfig(newConfig);

        if (!validationResult.valid) {
          logger.error(`変更された設定ファイルに問題があります`, undefined, {
            errors: validationResult.errors || [],
            warnings: validationResult.warnings || [],
            formattedResult: JSON.stringify(validationResult, null, 2),
          });

          // 致命的なエラーがある場合は再読み込みをスキップ
          const criticalErrors = (validationResult.errors || []).filter(
            (error: any) => error.type === 'schema' || error.type === 'dependency'
          );
...
// 編集または明示的に有効化されたサーバーの再接続を管理
const pendingReconnects = new Set<string>();

// 編集されたサーバーの再接続をスケジュール
export function scheduleReconnect(serverName: string) {
  if (!pendingReconnects.has(serverName)) {
    pendingReconnects.add(serverName);
    
    // 5秒後に再接続を試行
    setTimeout(async () => {
      pendingReconnects.delete(serverName);
      const config = await loadConfig();
      const serverConfig = config.mcpServers[serverName];
      
      if (serverConfig && isServerEnabledForProfile(serverName, config)) {
        const client = mcpClients.get(serverName);
        if (client && client.status === 'error') {
          logger.info(`編集されたサーバーの再接続を試行`, { serverName });
          client.retryCount = 0;
          resetCircuitBreaker(serverName);
          await connectToMCPServer(serverName, serverConfig, 0).catch((error) => {
            logger.debug(`再接続に失敗`, { serverName });
          });
        }
      }
    }, 5000);
...
async function main() {
  ...
  if (!validationResult.valid) {
    ...
    logger.warn(`設定ファイルの検証で警告または提案があります`, {
      warnings: validationResult.warnings || [],
      suggestions: (validationResult as any).suggestions || [],
      formattedResult: JSON.stringify(validationResult, null, 2),
    });
  } else {
  ...
  const monitoringInterval = setInterval(() => {
    ...
    logger.debug(
      `[接続プール統計] 総接続数: ${stats.totalConnections}, アクティブ: ${stats.activeConnections}, アイドル: ${stats.idleConnections}, 待機中: ${stats.waitingRequests}`
    );
  }, 60000); // 1分ごと
